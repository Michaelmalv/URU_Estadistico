...
export default function EconomiaView({ fixedCategoria = null, hideCategorySelector = false }) {
  ...
  const getProyectosFiltrados = useCallback(() => {
    const activeCat = fixedCategoria || categoriaUi;

    return proyectos.filter(p => {
      // Ocultar Escalinatas Rocafuerte de la pestaña de economía
      if (p.nombre === 'Escalinatas Rocafuerte') return false;

      // Filtrar por categoría
      if (activeCat !== 'Todas') {
        if (activeCat === 'Rehabilitación del Espacio Público' || activeCat === 'Rehabilitación de Espacio Público' || activeCat === 'Rehabilitación del Espacio Público y Centro Histórico') {
          ...
          if (p.categoria !== (catMap[activeCat] || activeCat)) return false;
        }
      }
  ...
  }, [proyectos, categoriaUi, fixedCategoria, fechaFilter]);
  ...
  if (loadingProjects) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '300px' }}>
        <p style={{ color: 'var(--text-muted)' }}>Cargando datos económicos...</p>
      </div>
    );
  }
    ...
    return (
      <div className="card" style={{ textAlign: 'center', padding: '3rem 1.5rem', marginTop: '1rem' }}>
        <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', margin: 0 }}>
          No se registran indicadores económicos específicos para esta área de intervención.
        </p>
      </div>
    );
  }
