...
export default function SeguridadView({ fixedCategoria = null, hideCategorySelector = false }) {
        ...
        if (data.success) {
          ...
          if (fixedCategoria) {
            if (fixedCategoria === 'Rehabilitación del Espacio Público' || fixedCategoria === 'Rehabilitación del Espacio Público y Centro Histórico') {
              cats = cats.filter(c => c === 'Plan de Rehabilitación Centro Histórico de Quito' || c === 'Recuperación de espacios público');
            } else {
              cats = cats.filter(c => c === fixedCategoria);
            }
          }
  ...
  useEffect(() => {
    ...
    if (fixedCategoria) {
      if (fixedCategoria === 'Rehabilitación del Espacio Público' || fixedCategoria === 'Rehabilitación del Espacio Público y Centro Histórico') {
        availableProjects = proyectos.filter(
          p => (p.categoria === 'Plan de Rehabilitación Centro Histórico de Quito' || p.categoria === 'Recuperación de espacios público') &&
               seguridadData.some(s => s.proyecto_id === p.id)
        );
      } else {
        availableProjects = proyectos.filter(
          p => p.categoria === fixedCategoria && seguridadData.some(s => s.proyecto_id === p.id)
        );
      }
  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '300px' }}>
        <p style={{ color: 'var(--text-muted)' }}>Cargando datos de seguridad...</p>
      </div>
    );
  }

  // Filtrado de proyectos disponibles para el selector
  const availableProjectsForSelect = proyectos
    .filter(p => {
      if (fixedCategoria) {
        if (fixedCategoria === 'Rehabilitación del Espacio Público' || fixedCategoria === 'Rehabilitación del Espacio Público y Centro Histórico') {
          return p.categoria === 'Plan de Rehabilitación Centro Histórico de Quito' || p.categoria === 'Recuperación de espacios público';
        }
        return p.categoria === fixedCategoria;
      }
    ...
    return (
      <div className="card" style={{ textAlign: 'center', padding: '3rem 1.5rem', marginTop: '1rem' }}>
        <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', margin: 0 }}>
          No se registran indicadores de seguridad específicos para esta área de intervención.
        </p>
      </div>
    );
  }
