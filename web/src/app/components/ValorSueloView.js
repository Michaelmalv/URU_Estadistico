...
const CATEGORIAS_LIST = ['Todas', 'Rehabilitación del Espacio Público y Centro Histórico', 'Corredores Vivos', 'Zonas Metro'];
...
export default function ValorSueloView({ fixedCategoria = null, hideCategorySelector = false }) {
  ...
  // Normalizar categoría inicial
  const getMappedInitialCat = () => {
    if (!fixedCategoria) return 'Todas';
    if (fixedCategoria === 'Rehabilitación del Espacio Público' || fixedCategoria === 'Rehabilitación del Espacio Público y Centro Histórico') {
      return 'Rehabilitación del Espacio Público y Centro Histórico';
    }
    return fixedCategoria;
  };

  const [selectedCategoria, setSelectedCategoria] = useState(getMappedInitialCat());
  const [selectedProyecto, setSelectedProyecto] = useState('');

  useEffect(() => {
    if (fixedCategoria) {
      if (fixedCategoria === 'Rehabilitación del Espacio Público' || fixedCategoria === 'Rehabilitación del Espacio Público y Centro Histórico') {
        setSelectedCategoria('Rehabilitación del Espacio Público y Centro Histórico');
      } else {
        setSelectedCategoria(fixedCategoria);
      }
    }
  }, [fixedCategoria]);
    ...
    return (
      <div className="card" style={{ textAlign: 'center', padding: '3rem 1.5rem', marginTop: '1rem' }}>
        <p style={{ color: 'var(--text-muted)', fontSize: '1.05rem', margin: 0 }}>
          No se registran avalúos de valor de suelo específicos para esta área de intervención.
        </p>
      </div>
    );
  }
